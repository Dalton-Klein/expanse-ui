...
export class DebugTerrainGenerator {
  ...
  static generateDebugTerrain(
    ...
    for (let chunkX = 0; chunkX < gridSize; chunkX++) {
      for (let chunkZ = 0; chunkZ < gridSize; chunkZ++) {
        ...
        const chunk =
          DebugTerrainGenerator.generateDebugChunk(
            position,
            config
          );
        chunks.push(chunk);
  ...
  static generateDebugChunk(
    ...
    switch (config.generation.debugPattern) {
      case DebugPattern.FLAT:
        DebugTerrainGenerator.generateFlatPattern(
          chunk,
          config
        );
        break;
      case DebugPattern.CHECKERBOARD:
        DebugTerrainGenerator.generateCheckerboardPattern(
          chunk,
          config
        );
        break;
      case DebugPattern.STEPPED:
        DebugTerrainGenerator.generateSteppedPattern(
          chunk,
          config
        );
        break;
   ...
   */
  private static generateFlatPattern(
    chunk: ChunkData,
    config: TerrainConfig
  ): void {
   ...
   */
  private static generateCheckerboardPattern(
    chunk: ChunkData,
    config: TerrainConfig
  ): void {
    const baseHeight = 1;
