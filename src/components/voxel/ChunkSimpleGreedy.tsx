...
interface ChunkProps {
  ...
  getVoxelAtWithLODCheck?: (worldX: number, worldY: number, worldZ: number, requestingChunkLOD: number) => VoxelType;
}

export default function ChunkSimpleGreedy({ data, getVoxelAt, getVoxelAtWithLODCheck }: ChunkProps) {
  const { geometry, faceCount } = useMemo(() => {
    ...
    const chunkWidth = data.lodScale ? Math.ceil(CHUNK_SIZE / lodScale) : CHUNK_SIZE;
    const chunkHeight = data.lodScale ? Math.ceil(CHUNK_HEIGHT / lodScale) : CHUNK_HEIGHT;
    ...
    if (!data.voxels || !Array.isArray(data.voxels)) {
      ...
      return { geometry: new THREE.BufferGeometry(), faceCount: 0 };
    }
    ...
    const getVoxelLocal = (x: number, y: number, z: number): VoxelType => {
      ...
      if (x >= 0 && x < chunkWidth && y >= 0 && y < chunkHeight && z >= 0 && z < chunkWidth) {
        ...
        if (data.voxels[x] && data.voxels[x][y] && data.voxels[x][y][z]) {
      ...
      const isAtChunkBoundary = (x < 0 || x >= chunkWidth || z < 0 || z >= chunkWidth);
      ...
      if (getVoxelAtWithLODCheck) {
        const worldX = data.position[0] * CHUNK_SIZE + (x * lodScale);
        const worldY = y * lodScale;
        const worldZ = data.position[2] * CHUNK_SIZE + (z * lodScale);
        
        return getVoxelAtWithLODCheck(worldX, worldY, worldZ, lodScale);
      }
    ...
    for (let x = 0; x < chunkWidth; x++) {
      for (let y = 0; y < chunkHeight; y++) {
        for (let z = 0; z < chunkWidth; z++) {
          ...
          const worldX = data.position[0] * CHUNK_SIZE + (x * lodScale);
          const worldY = data.position[1] * CHUNK_SIZE + (y * lodScale);
          const worldZ = data.position[2] * CHUNK_SIZE + (z * lodScale);
          ...
          if (getVoxelLocal(x, y + 1, z) === VoxelType.AIR) {
            addFace(
              [worldX, worldY + lodScale, worldZ + lodScale],
              [worldX + lodScale, worldY + lodScale, worldZ + lodScale],
              [worldX + lodScale, worldY + lodScale, worldZ],
          ...
          if (getVoxelLocal(x, y - 1, z) === VoxelType.AIR) {
            addFace(
              ...
              [worldX + lodScale, worldY, worldZ + lodScale],
          ...
          // Right face (+X)
          if (getVoxelLocal(x + 1, y, z) === VoxelType.AIR) {
            addFace(
              [worldX + lodScale, worldY, worldZ],
              [worldX + lodScale, worldY + lodScale, worldZ],
              [worldX + lodScale, worldY + lodScale, worldZ + lodScale],
          ...
          if (getVoxelLocal(x, y, z + 1) === VoxelType.AIR) {
            addFace(
              [worldX + lodScale, worldY, worldZ + lodScale],
              [worldX + lodScale, worldY + lodScale, worldZ + lodScale],
              [worldX, worldY + lodScale, worldZ + lodScale],
    ...
    console.log(`Chunk [${data.position.join(', ')}]: Generated ${faceCount} faces (LOD${data.lodLevel || 1}, scale: ${lodScale})`);

    return { geometry, faceCount };
  }, [data, getVoxelAt, getVoxelAtWithLODCheck]);
